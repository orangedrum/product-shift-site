...
const AnimatedStat = ({ value, label, delay, color }: { value: string, label: string, delay: number, color: string }) => {
  ...
  return (
    <div ref={ref} className="relative group">
      ...
      <div 
        className="absolute top-2 left-1/3 w-24 h-24 rounded-full opacity-40 blur-xl animate-float z-10"
        style={{ 
          background: color,
      ...
      <div 
        className="absolute bottom-2 right-1/3 w-24 h-24 rounded-full opacity-40 blur-xl animate-float z-10 bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500"
        style={{ 
