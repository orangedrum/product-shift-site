...
const runTestHandler = async (req: express.Request, res: express.Response) => {
  ...
  try {
    ...
    // Log critical errors to the database
    if (supabaseUrl && supabaseServiceKey) {
      await supabase.from('error_logs').insert({ error_message: error.message, details: { stack: error.stack } });
    }
...
app.get('/api/admin/stats', async (req, res) => {
  ...
  try {
    ...
    if (usageError || waitlistError || errorsError) {
      throw new Error('Failed to fetch stats from database.');
    }

    res.json({ dailyUsage, waitlistCount, recentErrors });
  } catch (e: any) {
    res.status(500).json({ error: 'Failed to retrieve admin stats', details: e.message });
  }
});
