              ...
              <section>
                ...
                <div className="space-y-16">
                  {assets.filter((a: any) => a.type === 'case_study').map((cs: any, idx: number, filteredArr: any[]) => (
                    <div key={idx} id={`case-study-${idx}`} className="group border-2 border-black rounded-3xl overflow-hidden bg-white shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] scroll-mt-24">
                      ...
                      <div className="px-8 pb-8 pt-4 flex flex-col sm:flex-row items-center justify-between gap-4 no-print">
                         {idx < filteredArr.length - 1 ? (
                           <NeoButton 
                             onClick={() => document.getElementById(`case-study-${idx + 1}`)?.scrollIntoView({ behavior: 'smooth' })}
                             className="bg-indigo-600 text-white font-black w-full sm:w-auto"
                           >
                             Next Logic Proof <ArrowRight size={16} className="ml-1" />
                           </NeoButton>
